# 이지우 | Lee Jiwoo

> **AI가 답을 던지는 시대에, ‘왜?’를 묻는 개발자**
>
> 광운대학교 인공지능융합대학 정보융합학부 비주얼테크놀로지전공  
> GPA **4.32 / 4.50** · Major GPA **4.50 / 4.50**

📬 [dlwldn4824@naver.com](mailto:dlwldn4824@naver.com) · [GitHub](https://github.com/dlwldn4824) · [성장 기록 @due_study_archive](https://www.instagram.com/due_study_archive/)

---

## About Me

저는 정답을 빠르게 만드는 것보다 **“왜 이 문제가 생겼는지, 우리가 세운 전제가 맞는지”**를 먼저 확인하는 개발자입니다.

프로젝트에서는 `문제 정의 → 검증 → 구현 → 측정 → 회고`의 흐름을 반복합니다. AI 역시 무조건 사용하는 것이 아니라, **AI가 가장 잘하는 역할과 그렇지 않은 역할을 구분해 배치하는 것**을 중요하게 생각합니다.

그 과정과 실패, 프로젝트 회고를 [@due_study_archive](https://www.instagram.com/due_study_archive/)에 지속적으로 기록하고 있습니다.

---

## Academic

### GPA 4.32 / 4.50 · Major GPA 4.50 / 4.50

| 구분 | 내용 |
| --- | --- |
| 학교 | 광운대학교 |
| 학부 | 정보융합학부 |
| 전공 | 비주얼테크놀로지전공 |
| 재학 | 2024.03 ~ |
| 전체 평점 | **4.32 / 4.50** |
| 전공 평점 | **4.50 / 4.50** |
| 주요 전공 성과 | 기계학습 A+ 등 |

---

## 나는 정말 공부를 잘하는 사람인가?

### 아니요. 대신, 이해할 때까지 해보는 사람입니다.

좋은 성적을 받았지만 스스로를 **처음부터 공부를 잘했던 사람**이라고 생각하지 않습니다.

이해가 안 되는 내용이 나오면 논문 한 편을 읽고 끝내는 대신, 논문의 문제 정의부터 모델 구조, 실험 결과까지 다시 뜯어보며 **50~60장 분량의 PPT로 재구성**해 공부했습니다. 다른 사람에게 설명할 수 있을 정도가 되어야 제가 제대로 이해했다고 판단했습니다.

그래서 제 강점은 빠르게 외우는 능력보다,

- 모르는 내용을 그냥 넘기지 않는 것
- 복잡한 내용을 다시 구조화하는 것
- 결과보다 **왜 그런 결과가 나왔는지** 확인하는 것
- 배운 내용을 다음 프로젝트에 적용하는 것

에 가깝습니다.

> **좋은 성적은 결과였고, 끝까지 이해하려는 습관이 지금의 저를 만들었습니다.**

---

## Awards & Scholarships

| 시기 | 수상 · 장학 | 관련 프로젝트 / 기관 |
| --- | --- | --- |
| 2026.08.07 | **[파이썬 SW 활용 경진대회 심화 우수상](https://github.com/dlwldn4824/smart_icheon_care)** | [Smart Icheon Care](https://github.com/dlwldn4824/smart_icheon_care) |
| 2026.07.22 | **[소원 H.O.P.E 창의보조공학 경진대회 장려상](https://github.com/dlwldn4824/HOPE_organization)** | [또박또박](https://github.com/dlwldn4824/HOPE_organization) |
| 2026.07 | **[HUSS AI 경진대회 장려상](https://github.com/dlwldn4824/HOPE_organization)** | [또박또박](https://github.com/dlwldn4824/HOPE_organization) |
| 2026.06.26 | **[지능형 로봇 컨소시엄 대상](https://github.com/dlwldn4824/smart_icheon_care)** | [Smart Icheon Care](https://github.com/dlwldn4824/smart_icheon_care) |
| 2025.12 | **마이크로모듈 초급 SS급** | 지능형로봇사업단 |
| 2025.12 | **성적우수 장학금** | 광운대학교 |
| 2025.05.29 | **매치업 심화과정 우수상** | 실무보고서 · 포스터 |
| 2025.06 | **성적우수 장학금** | 광운대학교 |
| 2024.12 | **Dean's List** | 광운대학교 |
| 2024.12 | **창업동아리 경진대회 장려상** | 광운대학교 |
| 2024.09 | **성적우수 장학금** | 광운대학교 |
| 2024.05 | **글쓰기 대회 가작** | 산문 |

---

## Selected Projects

> 프로젝트를 **“무엇을 만들었나”보다 “어떤 문제를 발견했고, 어떤 기술을 왜 사용했나”**를 중심으로 정리했습니다.

| 프로젝트 | 문제 | 핵심 접근 | 결과 |
| --- | --- | --- | --- |
| **[Smart Icheon Care](https://github.com/dlwldn4824/smart_icheon_care)** | 넓은 지역을 소수 인력이 전수 순찰하기 어려움 | YOLO11s + ByteTrack 탐지 → Risk 우선순위 → HITL 행정 확정 | F1 **0.591**, mAP50 **0.439**, 약 **15.7 FPS** · 지능형 로봇 컨소시엄 **대상** · 파이썬 SW **심화 우수상** |
| **[또박또박 / HOPE](https://github.com/dlwldn4824/HOPE_organization)** · [서비스 레포](https://github.com/dlwldn4824/ttobak-ttobak) | 치료실 밖 가정에서 객관적인 발음 피드백이 부족 | Wav2Vec2-CTC · 음소 정렬 · PCC/PER 기반 발음 평가 + 게임 UX | 소원 H.O.P.E **장려상** · HUSS AI **장려상** · 팀장 |
| **[PinTime](https://github.com/dlwldn4824/pintime)** · [Live](https://pintime.vercel.app/) | 톡방마다 가능 시간을 다시 입력하고, 확정 후 캘린더에 또 등록 | 자연어 제약 파싱 → 후보 생성 → 사람 승인 → 일정 저장 | **라이브 프로토타입 배포**, 서비스화 진행 |
| **[답변등기](https://github.com/dlwldn4824/kb_AI_challenge)** | AI 상담 초안의 위험성과 전건 사람 검수 사이의 비효율 | AI는 초안·위험 선별, 봉인·발송은 SHA256/HMAC 기반 결정론 처리 | 위험 큐 **1,248 → 39**, 합성 Recall@39 **81.3%**, 게이트 차단·재생 **100%** |
| **[TM Multi-Agent](https://github.com/dlwldn4824/TM-MultiLayer-MentalHealth)** | 단일 LLM이 맥락·위험·생성·안전을 모두 담당하면 실패 지점 파악이 어려움 | 역할 분리 Multi-Agent + RAG + 조건부 Safety Revision | Safety **4.83**, Empathy **4.31**, Revision 약 **6%** |
| **[NeSy-SMP-repro](https://github.com/dlwldn4824/NeSy-SMP-repro)** | LLM만으로 해결하기 어려운 지식 제약·설명성과 논문↔코드 간극 | Neuro-Symbolic LTN·KG 논문 재현 및 감사 | Phase-3 gate / grounding 문서화 · 재현 진행 중 |
| **[piching_machine](https://github.com/dlwldn4824/piching_machine)** | 투구 제구 성공을 감이 아니라 확률과 피처로 설명하고 싶음 | Form / Intent / Exec / Clutch 피처 + CatBoost 실험 | Holdout BSS E20 ≈ **644.04**, 실험 진행 중 |
| **[band](https://github.com/dlwldn4824/band)** · [Band-Info](https://github.com/dlwldn4824/Band-Info) | 공연 예약·체크인·셋리스트가 여러 채널로 분산 | 운영 워크플로를 하나의 웹 제품으로 통합 | **320+ commits**, 동아리 **실사용** |

<details>
<summary><strong>More selected · TEMI / HCI / 전공</strong></summary>

| 프로젝트 | 문제 | 핵심 접근 | 결과 |
| --- | --- | --- | --- |
| **[mobile_robot_temi](https://github.com/dlwldn4824/mobile_robot_temi)** | 로봇 이동·촬영 명령의 즉시성과 UX 단절 | Socket.IO 실시간 명령 + Claude API | **18존 내비게이션**, 현장 연동 |
| **[TemiTellMe](https://github.com/dlwldn4824/TemiTellMe)** · [HCI-UX](https://github.com/dlwldn4824/HCI-UX) | 전시장 길찾기·대기·콘텐츠 경험이 분리 | WebView↔Temi SDK 브리지 + 이벤트 추천 | 현장 설문 **24명**, 8개 줄서기 프로그램 |
| **[기계학습](https://github.com/dlwldn4824/machine_learning)** | 디저트 소비를 감이 아닌 상권 데이터로 예측 | scikit-learn 기반 모델 비교·평가 지표 선정 | 실데이터 비교 실험 · **A+** |
| **[opensource_final](https://github.com/dlwldn4824/opensource_final)** | 셋리스트를 감으로 구성하는 비효율 | 552곡 기반 규칙 추천 · FastAPI · Docker | **552곡 코퍼스**, API 형태 구현 |

</details>

---

## Other Projects & Contributions

<details>
<summary><strong>† contributor · 전공 실습 펼치기</strong></summary>

| 프로젝트 | 분야 | 기여 |
| --- | --- | --- |
| **[WJVOX](https://github.com/KWwoojin/project)** † | Speech AI | 학습 → infer → 공유 흐름의 Frontend / UX |
| **[iM Ready](https://github.com/ik-s/iM-Ready)** · **[iM Shield](https://github.com/ik-s/iM-Shield)** † | 금융보안 AI | 보이스피싱 예방·사후 대응 서비스 FE 핵심 기여 |
| **[Cam-Kit](https://github.com/bhw119/Cam-Kit)** † | Startup | 캠퍼스 소분 구매 서비스 기획·개발 |
| **[환불원정대](https://github.com/kw-ic-info/25-team-refund-ranger)** † | Text Mining | 키워드·감정·토픽 분석 + Gemini 챗봇 「민심이」 · 팀장 |
| **[mobile-robot](https://github.com/yyeonseoo/mobile-robot)** † | Robotics | TEMI 로봇 팀 프로젝트 contributor |
| **[creative_py_project](https://github.com/dlwldn4824/creative_py_project)** | 전공 실습 | Python · CV · 서비스 구현 기초 |
| **[HCI](https://github.com/dlwldn4824/HCI)** | HCI / UX | HCI·UX 평가 및 인터랙션 설계 |
| **[9th_web](https://github.com/dlwldn4824/9th_web)** | Web | UMC Web 실습 |

`†` 타 계정 또는 조직 레포 contributor

</details>

---

## Research & Learning

### 논문을 읽는 데서 끝내지 않고, 다시 설명할 수 있을 때까지

연구나 새로운 기술을 공부할 때는 논문을 단순히 요약하지 않습니다.

**Problem → 기존 방법 → Method → 실험 → 한계 → 내가 다시 검증할 부분**으로 재구성하고, 필요한 경우 **한 논문을 50~60장 규모의 PPT로 다시 만드는 방식**으로 공부합니다.

이 과정은 이후 [NeSy-SMP 논문 재현](https://github.com/dlwldn4824/NeSy-SMP-repro), [TM Multi-Agent](https://github.com/dlwldn4824/TM-MultiLayer-MentalHealth), [기계학습 프로젝트](https://github.com/dlwldn4824/machine_learning) 등에서 **“기술을 사용하는 것”보다 “왜 그렇게 동작하는지 검증하는 습관”**으로 이어졌습니다.

---

## Experience

### LG AIMERS 9기

**2026.07 ~**

- [piching_machine](https://github.com/dlwldn4824/piching_machine)
- 투구 제구 성공 확률 예측 프로젝트 진행

### 광운대학교 대학혁신 서포터즈

**2025.06 ~ 현재**

- 대학 프로그램 홍보
- 재학생 지원 및 콘텐츠 제작

---

## Teaching & Mentoring

### KT 희망나눔재단 랜선나눔캠퍼스

**2026.07 ~ 2026.09**

- 중학생 대상 AI·데이터 교육
- AI 코디네이터, 머신러닝, 프로젝트 설계 멘토링

### 에듀탑 온라인 화상과외

**2024.09 ~ 2025.12**

- 초·중·고 수학 약 10명 맞춤 지도

### 월계 채움학원

**2024.07 ~ 2025.12**

- 수학 조교
- 예비 고3 15~20명 풀이 및 상담 지원

### 개인 과외 · 자란다 멘토

**2023.08 ~ 2025.06**

- 초등 수학
- 고등 국어·수학
- 어린이 영어 보육

---

## What I Value

### 01. 문제부터 의심합니다.

주어진 문제를 바로 풀기보다 **“이게 정말 해결해야 하는 문제인가?”**부터 확인합니다.

### 02. AI가 꼭 필요한지 먼저 생각합니다.

AI 경진대회라고 모든 기능을 AI에 맡길 필요는 없습니다. **AI가 가장 잘하는 역할을 맡기고, 정확성이 필요한 부분은 다른 기술으로 보완**합니다.

### 03. 작동하는 증거를 만듭니다.

아이디어를 설명하는 것에서 끝내지 않고 **MVP, 데이터, 실험 결과**로 가설을 확인합니다.

### 04. 프로젝트가 끝나면 다시 시작합니다.

수상 여부와 관계없이 결과를 분석하고 다른 팀을 인터뷰하거나 회고를 남깁니다. 그 기록은 다음 프로젝트의 기준이 됩니다.

---

## 한 문장으로

> **저는 공부를 잘해서 좋은 결과를 만드는 사람이 아니라, 모르는 것을 끝까지 이해하고 다음 시도에서 더 나은 답을 만드는 사람입니다.**

---

## Contact

- **Email** · [dlwldn4824@naver.com](mailto:dlwldn4824@naver.com)
- **GitHub** · [github.com/dlwldn4824](https://github.com/dlwldn4824)
- **Archive** · [@due_study_archive](https://www.instagram.com/due_study_archive/)

---

## English (short)

- Visual Technology / Information Convergence, Kwangwoon University · GPA **4.32 / 4.50** · Major GPA **4.50 / 4.50**
- Not a “naturally gifted” student — rebuilds papers into 50–60 slide decks until explainable
- Loop: **problem → validate → build → measure → reflect**; place AI only where it fits
- Highlights: [Smart Icheon Care](https://github.com/dlwldn4824/smart_icheon_care) · [또박또박](https://github.com/dlwldn4824/HOPE_organization) · [PinTime](https://github.com/dlwldn4824/pintime) · [답변등기](https://github.com/dlwldn4824/kb_AI_challenge)
